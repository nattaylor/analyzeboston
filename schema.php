<?php
	...
	$package_list = json_decode(curl_exec($ch))->result->results;
	curl_close($ch);
	...
	foreach ($package_list as $package) {
		...
		$schema[$package->resources[0]->id] = array("fields" => array(), "title" => $package->title);
		foreach($result->result->fields as $field) {
			array_push($schema[$package->resources[0]->id]["fields"], $field->id);
		}
